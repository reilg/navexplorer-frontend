...
class MarketController extends AbstractController
{
    ...
    public function chart(Request $request, SerializerInterface $serializer, CoinGeckoApi $coinGeckoApi)
    {
        ...
        foreach ($response['prices'] as $key => $price) {
            ...
            $prices[$time] = [
                'time' => $time,
                'usd' => $this->getParameter('navcoin.network') == "mainnet" ? $price[1] : $price[1],
                'btc' => $this->getParameter('navcoin.network') == "mainnet" ? $btcResponse['prices'][$key][1] * 100000000 : $btcResponse['prices'][$key][1] * 100000000,
            ];
        }
